...
/Author   (Edited by Sebastian Suarez, based on the Michael Goerz work)
...
% Header
%\hrule\vskip 3pt
\title{VIM CHEAT SHEETS}

\vskip 0.5cm
\cm{:viusage }{Show a summary of all commands }
\cm{:h keyword}{Open help for keyword}
\cm{:ter}{Open a terminal window}
\cm{:sav file}{Save file as}
\cm{k}{Open man page for word under the cursor}


\section{\bf{{Movements}}}
\cm{h l k j }{character left, right; line up, down }
\cm{b w }{start of word/token left, right }
...
\cm{0 \^\ g\_ \$ }{beginning, first character, last character and end of line}
\cm{$n$G $n$gg }{Go to line $n$, when N is no indicated go to last line and first line}
\cm{:n}{Go to line $n$}
\cm{$n|$ }{Go to column $n$ of current line}
\cm{\% }{match of next brace, bracket, comment, :h matchpairs}
\cm{H M L}{beginning, middle, botton of {\it screen\/}}
\cm{ngj ngk }{move cursor down, up n lines}
...
\cm{ea}{Insert at the end of the word}
...
\cm{\ctrl n }{autocomplete next match before the cursor}
\cm{\ctrl p }{autocomplete next match after the cursor}
\cm{d$m$ }{delete text of movement command $m$}
\cm{J gJ }{join current line with next, without space}
\cm{s }{delete a character and substitute text}
\cm{S }{delete a line and substitute text}
\cm{xp }{tanspose two letters ab}


%\vskip 0.4cm
\section{\bf{Search \& Substitution}}
\cm{/pattern ?pattern}{search forward, backward for pattern}
...
\cm{>\% }{indent inner a block with () or {\tt\lapos} {\tt\rapos} }
\cm{>at}{indent inner block with $<$~$>$ tags}
\cm{n==}{re-indent n lines}
...
%\vskip 0.8cm
\section{\bf{Visual Mode \& Text Objects}}
\cm{v V \ctrl V }{highlight characters, lines, block}
\cm{o O}{exchange cursor pos with start/end of highlighting}
...
\vskip -0.2cm
\section{\bf{Undoing, Repeating \& Registers}}
\cm{u U }{undo last command, restore last changed line}
\cm{\ctrl r }{redo}
\cm{; ,}{Repeat last f,t,F,T movement backward, fordward}
\cm{. }{Repeat las change}
\cm{$n$.\ }{repeat last changes with count replaced by $n$ }
\cm{q$a$ }{record, append typed characters in register $a$}
\cm{q }{stop recording}
\cm{@$a$ }{execute the content of register $a$}
\cm{@@ }{repeat previous {\tt @} command}
\cm{:@$a$ }{execute register $a$ as an {\it Ex\/} command}
\cm{"$x$ }{use register $x$ for next delete, yank, put}
\cm{"$xy$ }{Yank into register $x$}
\cm{"$xp$ }{Paste content of register $x$}
\cm{"$+y$ }{Yank into the system clipboard}
\cm{"$+p$ }{Paste from the system clipboard}
\cm{\ctrl rx }{Insert the contents of regiser x}
\cm{:reg [$x$] } {show content registers/$x$ reg}



\vskip -0.2cm
\section{\bf{Copying (Yanking)}}
\cm{y$m$ }{yank the text of movement command $m$}
\cm{yy\or Y }{yank current line into register}
\cm{y$`m$ }{Yank text to position of the mark}
...
\section{\bf{Patterns (differences to Perl) ~~ {\tt :help pattern}}}
...
\section{\bf{Marks, Motions, and Tags}}
\cm{m$a$ }{mark current position with mark $a\in[a..Z]$}
\cm{`$a$ `$A$ }{go to mark $a$ in current, $A$ in any file}
...
\section{\bf{Multiple Files / Buffers (\enter)}}
\cm{:tabe }{open a new tab}
\cm{gt or:tabn }{move to the next tab}
\cm{gT or:tabp }{move to the previous tab}
\cm{\#gt or :tabm \#}{go to tab number \#}
\cm{:tab ball }{show buffer tablist}
\cm{:tabdo}{run the command on all tabs eg. :tabdo wq -save and close all the tabs}
\cm{:tabo}{close all tabs except for the current one}
\cm{:tabc }{close the current tab and all its windows}
...
\section{\bf{Scrolling \& Multi-Windowing}}
\cm{\ctrl d}{Move fordward $1/2$ page (\ctrl = Ctrl) }
\cm{\ctrl u}{Move backward $1/2$ page (\ctrl = Ctrl) }
...
\vskip -0.2cm
\section{\bf{Misc Ex Commands (\enter) ~~ \tt{:help holy-grail}}}
